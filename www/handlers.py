...
import re, time, json, logging, hashlib, base64, asyncio, random
...
async def addUser(request):
    print('addUser is executing...')
    print(request.query, request.query_string)
    r_str = str(random.random())[2:12]
    u = User(name='user_%s' % r_str, email='user_%s@example.com' % r_str, password='123456', admin=False,
             image='about:blank')
    await u.save()
    return 'success'
